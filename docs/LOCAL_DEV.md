...
- [ ] Click **Set part…** in the selection bar — a plain `<select>` of every `Part` value. Picking
      one tags every currently-selected photo with it; **— clear part —** removes the tag instead.
      There is no visual car picker of any kind (3D or 2D diagram) — both were removed; a plain
      dropdown is the only way to set a part, on Upload and on the vehicle page alike.
- [ ] At the top of a vehicle page, **Photo coverage** lists a small button per part that has at
      least one photo, each showing that part's photo count. Click one to filter the photo grid to
      it; click "Show all" to clear the filter. Parts with zero photos aren't listed — with no
      diagram there is no "everything, shaded or not" surface to show them against.
...
### The panel-beater parts

`Models/Part.cs` carries far more members than a workshop photo actually needs a hotspot for —
detail trim (A/B/C-pillars, drip rails, handles, wipers, plates, exhaust tips, an antenna, a fuel
filler) that briefly had real 3D click regions when this app had a 3D/2D car picker, before both
were removed in favour of a plain dropdown. None of that granularity was reverted: the enum is
append-only and every value that ever shipped stays valid forever (`PartTests.NumericValuesNeverChange`
pins each one), so all of it is still there, just reachable only via the dropdown now, the same as
the six parts (`Interior`, `EngineBay`, `LoadArea`, `Odometer`, `Undercarriage`, `Other`) that never
had a visual hotspot of any kind even when the picker existed.

- [ ] Every `Part` value appears once, and only once, in the tag-photo `<select>` on both Upload
      and the vehicle page, and in the "Set part…" bulk-tag dropdown.
- [ ] A photo can be tagged to any of them, niche or common, and it round-trips correctly through
      `PhotoTaggingService` and back out to the photo tile's badge.

### Damage marking

The always-rendered **Mark damage** panel (works even before any photo exists) is a part dropdown
plus an **Open** button — picking a part and clicking it opens a popup: the vehicle's real tagged
photo if one exists for that part, otherwise a text-only fallback (there is no diagram any more to
fall back to a zoomed shape from). Either way, clicking anywhere on a shown photo that isn't an
existing pin starts a new one; existing pins render as small dots with a tooltip, never burned into
the image itself. A second, permanent "Damage marks" list on the vehicle page is the browsable
record — part, thumbnail-or-"(no photo)", note, author, timestamp.

The dropdown+button trigger (and the popup itself) is shown to every role, not just uploaders —
opening it is also how anyone browses a part's existing marks. Whether it lets you actually *add* a
new one is gated inside the popup by `window.__canMarkDamage` (Staff/Admin only), not by hiding the
trigger.

- [ ] Pick a part with **no** tagged photo, click Open — popup shows the text-only fallback
      ("No photo tagged to this part yet — you can still record a note").
- [ ] Click inside it — a pin drops at the centre and a note field appears; Save → the mark appears
      both in the popup and in the "Damage marks" list.
- [ ] Tag a photo to that same part, reopen the popup for it — now shows that photo instead of the
      fallback; the earlier fallback-anchored mark is untouched (still listed, `photoId` still
      null).
- [ ] Delete the photo a mark was anchored to — that mark disappears too (cascade); any
      fallback-anchored marks for the same part are unaffected.
...
- [ ] Pick a button-only part (one that never had a diagram/3D hotspot even before, e.g. Interior)
      ...
      was already true before the diagram was removed; confirming it still holds is really
      confirming the fallback path itself, not anything part-specific.
